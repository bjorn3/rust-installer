            ...
            match path.components().next().unwrap() {
                Component::Prefix(prefix_component) => {
                    match prefix_component.kind() {
                        ...
                        Prefix::Disk(_disk) => Path::new("\\\\?\\")
                            .join(prefix_component.as_os_str())
                            .join(normalize_rest(path)),
                    }
                }
...
impl FromStr for LongPath {
    type Err = Infallible;

    fn from_str(s: &str) -> Result<LongPath, Infallible> {
        Ok(LongPath::new(s.into()))
    }
}
